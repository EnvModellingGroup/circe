...
manning_drag = params.manning_drag
...
blend_dist = params.blend_dist

# first deal with bathymetry   
with timed_stage('initialising bathymetry'):
    if len(params.rasters) > 1:
    bathy = hrds.HRDS(params.rasters[0], rasters=params.rasters[1:], distances=params.distances)
    else:
        hrds.HRDS(params.rasters[0])
